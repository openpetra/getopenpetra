...
#
# This should work on CentOS 7 and CentOS Stream 8 and 9, Fedora 35 and 36
...
install_ubuntu()
{
	...
	if [[ "$install_type" == "devenv" ]]; then
		# for cypress tests
		apt-get -y install libgtk2.0-0 libgtk-3-0 libnotify-dev libgbm-dev libgconf-2-4 libnss3 libxss1 libasound2 libxtst6 xauth xvfb gconf2 libgdk-pixbuf2.0-0 libqt5gui5 libegl1 libegl-mesa0 || exit -1
	fi
	# for printing bar codes
	curl --silent --location https://github.com/Holger-Will/code-128-font/raw/master/fonts/code128.ttf > /usr/share/fonts/truetype/code128.ttf
...
install_openpetra()
{
	...
	if [ -f /etc/os-release ]; then
		...
		if [[ "$OS_FAMILY" == "Fedora" ]]; then
			if [[ "$VER" != "7" && "$VER" != "8" && "$VER" != "9" && "$VER" != "35" && "$VER" != "36" ]]; then
